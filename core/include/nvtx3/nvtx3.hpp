...
/* Temporary helper #defines, removed with #undef at end of header */
     ...
     * that does not work with domain::get specialization for domain::global,
...
NVTX3_INLINE_IF_REQUESTED namespace NVTX3_VERSION_NAMESPACE {
  ...
  class message {
   public:
    ...
    NVTX3_CONSTEXPR_IF_CPP14 message(
        registered_string_in<D> const& msg) noexcept
  ...
  class optional_scoped_range_in {
   public:
    ...
    optional_scoped_range_in& operator=(optional_scoped_range_in const&) =
        delete;
  ...
  inline void mark(Args const&... args) noexcept {
     ...
     *
     * Constructs a static `registered_string_in` using the name of the
     * immediately enclosing function returned by `__func__` and constructs a
     ...
     * `NVTX3_V1_FUNC_RANGE_IF_IN(D, C)` only generates a range if the given
  ...
  /**
   * @brief Convenience macro for generating a range in the global domain from
  ...
  /**
   * @brief Convenience macro for generating a range in the global domain from
   * the lifetime of a function if the given boolean expression evaluates to
   * true.
   *
