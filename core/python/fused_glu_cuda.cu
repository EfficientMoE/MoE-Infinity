// fused_silu_gemm_bf16.cu - Fused Tiled GEMM + SiLU with Shared Memory (CUDA Kernel + PyTorch Binding for BF16)
...
inline void throwOnCublasError(cublasStatus_t error, const char* file, int line,
    ...
    if (error != CUBLAS_STATUS_SUCCESS) {
        ...
        ss << "CUBLAS error " << error << " (" << errorString << ") at " << file << ":" << line
...
torch::Tensor linear_cublaslt_bf16(torch::Tensor input, torch::Tensor weight) {
    ...
    // // Epilogue with bias
    // cublasLtMatmulDescSetAttribute(op_desc, CUBLASLT_MATMUL_DESC_BIAS_POINTER, &bias.data_ptr<float>(), sizeof(void*));
    ...
    CUBLAS_CHECK(cublasLtMatmul(
        ...
        op_desc,
        &alpha,
        input.data_ptr<at::BFloat16>(), a_desc,
        weight.data_ptr<at::BFloat16>(), b_desc,
        &beta,
        output.data_ptr<at::BFloat16>(), c_desc,
        output.data_ptr<at::BFloat16>(), c_desc,
        nullptr, nullptr, 0, 0));
...
__global__ void fused_silu_gemm_kernel(
    ...
    for (int tile_k = 0; tile_k < D; tile_k += TILE_DIM) {
        ...
        if (tiled_row < B && (tile_k + threadIdx.x) < D)
            tileA[threadIdx.y][threadIdx.x] = hidden[tiled_row * D + tile_k + threadIdx.x];
        else
            tileA[threadIdx.y][threadIdx.x] = __float2bfloat16(0.0f);

        if (tiled_col < G && (tile_k + threadIdx.y) < D)
            tileB[threadIdx.y][threadIdx.x] = gate_proj[tiled_col * D + tile_k + threadIdx.y];
        else
...
at::Tensor fused_silu_gemm(torch::Tensor hidden, torch::Tensor gate_proj) {
    ...
    TORCH_CHECK(hidden.size(1) == gate_proj.size(1), "Inner dimensions must match");
...
// at::Tensor fused_silu_gemm_cublas(torch::Tensor hidden, torch::Tensor gate_proj) {
//     TORCH_CHECK(hidden.dim() == 2 && gate_proj.dim() == 2, "Expected 2D tensors");
//     TORCH_CHECK(hidden.size(1) == gate_proj.size(1), "Inner dimensions must match");
...
//     const __nv_bfloat16* Amat = reinterpret_cast<const __nv_bfloat16*>(hidden.data_ptr<at::BFloat16>());
//     const __nv_bfloat16* Bmat = reinterpret_cast<const __nv_bfloat16*>(gate_proj.data_ptr<at::BFloat16>());
//     __nv_bfloat16* C = reinterpret_cast<__nv_bfloat16*>(output.data_ptr<at::BFloat16>());
...
//     // TORCH_CHECK(props.major >= 8, "BF16 is only supported on Ampere or newer GPUs");

//     // TORCH_CHECK(hidden.stride(1) == 1, "hidden must be contiguous in last dim");
//     // TORCH_CHECK(gate_proj.stride(1) == 1, "gate_proj must be contiguous in last dim");
//     // TORCH_CHECK(output.stride(1) == 1, "output must be contiguous in last dim");
...
at::Tensor fused_silu_gemm_cublas(torch::Tensor hidden, torch::Tensor gate_proj) {
    ...
    const __nv_bfloat16* A = reinterpret_cast<const __nv_bfloat16*>(hidden.data_ptr<at::BFloat16>());
    const __nv_bfloat16* Bmat = reinterpret_cast<const __nv_bfloat16*>(gate_proj.data_ptr<at::BFloat16>());
    __nv_bfloat16* C = reinterpret_cast<__nv_bfloat16*>(output.data_ptr<at::BFloat16>());
    ...
    cublasStatus_t status = cublasGemmEx(
        ...
        G, B, D,           // m, n, k
        &alpha,
        Bmat, CUDA_R_16BF, G,  // lda = G
        A,    CUDA_R_16BF, D,  // ldb = D
        &beta,
        C,    CUDA_R_16BF, G,  // ldc = G
        CUDA_R_32F,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("linear_cublaslt_bf16", &linear_cublaslt_bf16, "Linear layer with cuBLAS LT (BF16)");
    m.def("fused_silu_gemm", &fused_silu_gemm, "Fused GEMM + SiLU (CUDA, BF16)");
    m.def("fused_silu_gemm_cublas", &fused_silu_gemm_cublas, "Fused GEMM + SiLU (CUDA, BF16, CUBLAS)");
}