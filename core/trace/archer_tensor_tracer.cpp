...
void ArcherTensorTracer::ClearRequestID() {
  if (traces_.size() > 1000) {
    traces_.erase(request_id_);
  }
  request_id_ = UINT64_MAX;
}

std::vector<std::uint32_t> ArcherTensorTracer::GetCandidates(
  ...
  for (auto& req_item : traces_[request_id_]) {
    ...
    for (auto& hist_item : traces_) {
      ...
      for (auto& value_item : value) {
        ...
        std::set_intersection(req_tensors.begin(), req_tensors.end(),
                              hist_tensors.begin(), hist_tensors.end(),
                              std::back_inserter(overlap));

        std::vector<std::uint32_t> total;
        std::set_union(req_tensors.begin(), req_tensors.end(),
                       hist_tensors.begin(), hist_tensors.end(),
                       std::back_inserter(total));

        double prob = static_cast<double>(overlap.size()) / total.size() /
                      (layer_id - req_layer + 1);

        std::vector<std::pair<std::uint32_t, std::vector<std::uint32_t>>>
            candidate_value;
        for (auto& pair : value) {
          if (pair.first > layer_id) {
            candidate_value.push_back(pair);
          }
        }

        if (!candidate_value.empty()) {
          candidates.push_back({candidate_value, prob});
        }
        break;
      }
    }
  ...
  for (auto& item : tensor_probs) {
    ...
    // if (layer < layer_id + 2) continue; // skip the layers that are too close
    ...
    layer_tensor_probs_vec.erase(
        std::remove_if(layer_tensor_probs_vec.begin(),
                       layer_tensor_probs_vec.end(),
                       [](const std::pair<std::uint32_t, double>& pair) {
                         return pair.second < 0.01;
                       }),
        layer_tensor_probs_vec.end());

    if (layer_tensor_probs_vec.empty()) {
      continue;
    }

    std::sort(layer_tensor_probs_vec.begin(), layer_tensor_probs_vec.end(),
    ...
    for (std::uint32_t i = 0;
         i < std::min(layer_tensor_probs_vec.size(), width); ++i) {
