...
void Node::SetDevice(const torch::Device& target_device, bool on_demand,
  ...
  if (kCudaStreamH2D == NULL) {
    auto cudaError =
        cudaStreamCreateWithFlags(&kCudaStreamH2D, cudaStreamNonBlocking);
    if (cudaError != cudaSuccess) {
      ARCHER_LOG_ERROR("cudaStreamCreate failed: {}",
  ...
  if (target_device == DISK_DEVICE) {
    ...
    if (host_memory_ptr == nullptr && device_memory_ptr == nullptr) {
      ...
      ARCHER_LOG_DEBUG("SetModuleMemoryFromDisk time: {} us",
                       end_time - start_time);
    }
    ...
    if (target_device.is_cuda()) {
      ...
      auto end_time = MCIROSECONDS_SINCE_EPOCH;
      ARCHER_LOG_DEBUG("SetModuleCudaMemoryFromCPU time: {} us",
                       end_time - start_time);
    }
    ...
    if (target_device.is_cpu() && device.is_cuda()) {
      ...
      device_memory_ptr = nullptr;
      auto end_time = MCIROSECONDS_SINCE_EPOCH;
      ARCHER_LOG_DEBUG("SetModuleMemoryFromCuda time: {} us",
                       end_time - start_time);
    }
  }
...
void ArcherTopologyHandle::SetNodeVisitCounts(
  ...
  if (visit_counts.size() != num_nodes) {
    ARCHER_LOG_ERROR("visit_counts size {} not equal to num_nodes {}",
...
void ArcherTopologyHandle::SetChildVisitCounts(
  ...
  if (visit_counts.size() != (num_layers - 1) * num_experts * num_experts) {
    ARCHER_LOG_ERROR("visit_counts size {} not equal to num_layers {}",
...
void ArcherTopologyHandle::InitializeTopology(
  ...
  ARCHER_LOG_DEBUG("InitializeTopology pipeline_.stages.size() {}",
  ...
  for (auto& node_ptr : all_nodes) {
    ARCHER_LOG_DEBUG("Node {} {} device {}", node_ptr->id, node_ptr->is_sparse,
...
void SetModuleMemoryFromDisk(std::vector<TensorID>& tensor_ids, void* host_ptr,
  ...
  for (const auto& tensor_id : tensor_ids) {
    ...
    ARCHER_LOG_DEBUG("SetModuleMemoryFromDisk tensor {}",
...
void SetModuleCudaMemoryFromCPU(std::vector<TensorID>& tensor_ids,
  ...
  // ARCHER_LOG_DEBUG("SetModuleCudaMemoryFromCPU {} tensors",
  // tensor_ids.size());
  std::int64_t param_size = 0;
  for (const auto& tensor_id : tensor_ids) {
    auto it = kTensorIndex->find(tensor_id);
    ARCHER_LOG_DEBUG("SetModuleCudaMemoryFromCPU tensor {} -> {}",
  ...
  // ARCHER_LOG_DEBUG("SetModuleCudaMemoryFromCPU {} tensors done",
  // tensor_ids.size());
}
...
void SetModuleMemoryFromCuda(std::vector<TensorID>& tensor_ids,
  ...
  for (const auto& tensor_id : tensor_ids) {
    ...
    auto it = kTensorIndex->find(tensor_id);
    ARCHER_LOG_DEBUG("SetModuleMemoryFromCuda tensor {}",
                     it->second.DebugString());
    it->second.tensor.set_data(
  ...
  // ARCHER_LOG_DEBUG("SetModuleMemoryFromCuda {} tensors done",
  // tensor_ids.size());
}
