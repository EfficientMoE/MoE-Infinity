...
__global__ void mxfp4_dequant_kernel(
    const uint8_t* packed, const uint8_t* scales, __nv_bfloat16* output,
    int rows, int packed_cols, int scale_cols, int block_size) {
  ...
  int exponent = static_cast<int>(
                     scales[static_cast<long>(row) * scale_cols +
                            col / block_size]) -
                 127;
