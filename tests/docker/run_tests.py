...
def run_pytest(marker_expr, label):
    """Run pytest with the given marker expression."""
    test_file = "tests/docker/test_io_integration.py"
    cmd = [
        sys.executable,
        "-m",
        "pytest",
...
def main():
    ...
    # Always run Tier 1
    tier1_ok = run_pytest("not cuda", "Tier 1: Threading & File I/O (no CUDA)")

    # Run Tier 2 if CUDA is available
    tier2_ok = True
    if cuda_available:
        tier2_ok = run_pytest(
            "cuda", "Tier 2: Pinned Memory & Tensor I/O (CUDA)"
        )
    else:
