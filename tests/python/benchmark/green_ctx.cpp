...
// ---------------------------------------------------------------------------
extern "C" int gc_setup_sm(int      device_id,
                            uint64_t *stream0_out,
                            uint64_t *stream1_out,
                            int      *sm0_out,
                            int      *sm1_out,
                            int      *total_out)
{
    ...
    CU_CHECK(cuDeviceGetDevResource(dev, &sm_res,
    ...
    CU_CHECK(cuDevSmResourceSplitByCount(result_arr, &nb,
                                         &sm_res, nullptr, 0, min_count));
    ...
    CU_CHECK(cuGreenCtxCreate(&g_gctx[0], desc0, dev,
                               CU_GREEN_CTX_DEFAULT_STREAM));
    CU_CHECK(cuGreenCtxCreate(&g_gctx[1], desc1, dev,
                               CU_GREEN_CTX_DEFAULT_STREAM));
...
// ---------------------------------------------------------------------------
extern "C" int gc_setup_sm_wq(int      device_id,
                               uint64_t *stream0_out,
                               uint64_t *stream1_out,
                               int      *sm0_out,
                               int      *sm1_out,
                               int      *total_out)
{
    ...
    unsigned int  nb        = 2;
    unsigned int  min_count = (unsigned int)(total_sms / 2);
    CU_CHECK(cuDevSmResourceSplitByCount(sm_parts, &nb,
                                         &sm_res, nullptr, 0, min_count));

    if (sm0_out) *sm0_out = (int)sm_parts[0].sm.smCount;
    if (sm1_out) *sm1_out = (int)sm_parts[1].sm.smCount;

    // ----- Work-queue configuration resource (CU_DEV_RESOURCE_TYPE_WORKQUEUE_CONFIG)
    // This is a configuration/scope hint, not a splittable resource.
    // Setting sharingScope = CU_WORKQUEUE_SCOPE_GREEN_CTX_BALANCED requests
    // non-overlapping WQ scheduling between the two green contexts.
