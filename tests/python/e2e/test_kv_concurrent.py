...
def concurrent_engine_bundle(
    ...
    offload_path = Path(tmp_path_factory.mktemp("e2e_concurrent")) / "offload"
    ...
    try:
        ...
        pytest.skip(f"Unable to initialize model for concurrent test: {exc}")
...
def test_kv_concurrent_4_requests_complete(concurrent_engine_bundle) -> None:
    engine, tokenizer = concurrent_engine_bundle

    request_count = 4
    num_blocks = engine.kv_cache.num_blocks
    block_size = engine.kv_cache.block_size
    blocks_per_request = max(1, num_blocks // request_count + 1)
    ...
    def submit_request(req_idx: int) -> None:
        try:
            _ = barrier.wait(timeout=10)
            engine.add_request(
                request_id=f"concurrent-{req_idx}",
    ...
    threads = [
        threading.Thread(target=submit_request, args=(idx,))
