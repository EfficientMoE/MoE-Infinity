...
def _device_and_dtype():
    if (
        os.environ.get("ROUNDTRIP_DEVICE", "").lower() == "cpu"
...
def _meta(
    is_prefill: bool, context: int, query: int, device
) -> AttentionMetadata:
    kv = context + query
    ...
    return AttentionMetadata(
        ...
        lengths=PagedBatchLengths(
            query_lengths=torch.tensor(
                [query], dtype=torch.int32, device=device
            ),
            query_offsets=torch.tensor(
                [0, query], dtype=torch.int32, device=device
            ),
...
def test_two_sequences_packed_in_one_prefill_match_stock_attention(family):
    ...
    q_len = max(lengths)
    hidden = torch.randn(
        len(lengths), q_len, cfg.hidden_size, device=device, dtype=dtype
    )
    positions = (
        torch.arange(q_len, device=device).unsqueeze(0).expand(len(lengths), -1)
    )
    cos, sin = rotary(hidden, positions)

    backend = PagedAttentionBackend(
        spec=KVCacheSpec(2, HEAD_DIM, dtype, BLOCK_SIZE),
        num_gpu_blocks=NUM_BLOCKS,
        num_layers=1,
        device=device,
    )
    offsets = [0, lengths[0], lengths[0] + lengths[1]]
    slots = list(range(0, lengths[0])) + list(
        range(2 * BLOCK_SIZE, 2 * BLOCK_SIZE + lengths[1])
    )
    ...
    with torch.no_grad():
        ...
        for row, length in enumerate(lengths):
            ...
            diff = (
                (out[row, :length].float() - ref[0].float()).abs().max().item()
            )
            assert torch.allclose(
                out[row, :length], ref[0], **_tol(dtype)
...
def test_two_layers_share_one_backend_without_aliasing(family):
    ...
    torch.manual_seed(3)
    shims = [
        Shim(cfg, layer_idx=i).to(device=device, dtype=dtype).eval()
        for i in range(2)
    ]
    stocks = [
        Stock(cfg, layer_idx=i).to(device=device, dtype=dtype).eval()
        for i in range(2)
    ]
    ...
    hidden = [
        torch.randn(1, total, cfg.hidden_size, device=device, dtype=dtype)
        for _ in range(2)
    ]
    ...
    def run(is_prefill, start, length):
        ...
        for layer in range(2):
            ...
            with torch.no_grad():
                ...
                Shim.set_paged_context(
                    backend, _meta(is_prefill, start, length, device)
                )
                try:
                    out, _ = shims[layer](
                        hidden_states=h,
            ...
            assert torch.allclose(
                out, ref, **tol
            ), f"{family} layer {layer} prefill={is_prefill} start={start}: max|diff|={diff:.4f}"
...
@pytest.mark.parametrize(
    "force_fallback", [False, True], ids=["kernel", "sdpa-fallback"]
)
def test_shim_prefill_and_decode_match_stock_attention(
    family, kv_heads, force_fallback, monkeypatch
):
    device, dtype = _device_and_dtype()
    if force_fallback:
        ...
        pytest.skip(
            "native kernel path needs CUDA; the fallback case covers CPU"
        )
    ...
    hidden_all = torch.randn(
        1, total, cfg.hidden_size, device=device, dtype=dtype
    )
