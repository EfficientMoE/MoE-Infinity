...
class TestGPTQDetection:
    def test_positive(self):
        from moe_store.parsing.gptq import is_gptq_quantized
...
class TestGPTQBitsAndGroupSize:
    def test_bits(self):
        from moe_store.parsing.gptq import get_gptq_bits

        assert get_gptq_bits(make_gptq_config()) == 4

    def test_group_size(self):
        from moe_store.parsing.gptq import get_gptq_group_size

        assert get_gptq_group_size(make_gptq_config()) == 128

    def test_defaults_when_no_config(self):
        from moe_store.parsing.gptq import get_gptq_bits, get_gptq_group_size
...
class TestGPTQPackedTensor:
    def test_qweight_detected(self):
        from moe_store.parsing.gptq import is_gptq_packed_tensor
...
class TestGPTQComponent:
    def test_all_components_detected(self):
        from moe_store.parsing.gptq import is_gptq_component
